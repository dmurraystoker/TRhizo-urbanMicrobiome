...
### Quantify Turnover & Nestedness: Root

\vspace{10pt}

```{r Quantify Turnover & Nestedness: Root, echo = TRUE, eval = FALSE}
## Convert abundance matrix to presence-absence matrix
root.ASV.presence.absence.matrix <- decostand(
  as_tibble(ASV.abundance.matrix) %>% slice(1:161),
  method = "pa"
)

## Calculate multi-site turnover and nestedness
root.turnover.nestedness.null.model <- oecosimu(
  root.ASV.presence.absence.matrix,
  nestfun = nestedbetajac,
  method = "r1",
  nsimul = 1000,
  parallel = 4
)
...
```{r Turnover & Nestedness Results: Root, echo = FALSE}
## Format data for presenting results and figure creation
root.turnover.nestedness.null.model.results <- tibble(
  Compoenent = c("Turnover", "Nestedness", "Jaccard"),
  Observed = root.turnover.nestedness.null.model$statistic,
  Simulated = root.turnover.nestedness.null.model$oecosimu$means,
  Effect_Size = root.turnover.nestedness.null.model$oecosimu$z,
  P_Value = root.turnover.nestedness.null.model$oecosimu$pval
)

kable(
  root.turnover.nestedness.null.model.results,
  booktabs = TRUE,
  caption = "Results of the turnover and nestedness analysis for root samples."
) %>%
...
\newpage
### Quantify Turnover & Nestedness: Soil

\vspace{10pt}

```{r Quantify Turnover & Nestedness: Soil, echo = TRUE, eval = FALSE}
## Convert abundance matrix to presence-absence matrix
soil.ASV.presence.absence.matrix <- decostand(
  as_tibble(ASV.abundance.matrix) %>% slice(161:322),
  method = "pa"
)

## Calculate multi-site turnover and nestedness
soil.turnover.nestedness.null.model <- oecosimu(
  soil.ASV.presence.absence.matrix,
  nestfun = nestedbetajac,
  method = "r1",
  nsimul = 1000,
  parallel = 4
...
```{r Turnover & Nestedness Results: Soil, echo = FALSE}
## Format data for presenting results and figure creation
soil.turnover.nestedness.null.model.results <- tibble(
  Compoenent = c("Turnover", "Nestedness", "Jaccard"),
  Observed = soil.turnover.nestedness.null.model$statistic,
  Simulated = soil.turnover.nestedness.null.model$oecosimu$means,
  Effect_Size = soil.turnover.nestedness.null.model$oecosimu$z,
  P_Value = soil.turnover.nestedness.null.model$oecosimu$pval
)

kable(
  soil.turnover.nestedness.null.model.results,
  booktabs = TRUE,
  caption = "Results of the turnover and nestedness analysis for soil samples."
) %>%
  kable_styling(latex_options = "HOLD_position", "striped")
```
...
save(
  ...
  weighted.UniFrac.multivariate.dispersion.by.ISC.GAM,
  file = "data_analysis/7-diversity_composition_analyses/diversity_composition-reduced_workspace.RData"
)
```
...
save.image("data_analysis/7-diversity_composition_analyses/diversity_composition-workspace.RData")
```
